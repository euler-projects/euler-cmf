...
CREATE TABLE CMF_ATTACHMENT
	(
	  ID           VARCHAR(36)  NOT NULL PRIMARY KEY COMMENT '附件ID(文件归档ID)',
	  ...
	  CONSTRAINT FK_ATTACHMENT_POST_ID FOREIGN KEY (POST_ID) REFERENCES CMF_POST (ID)
	)
	ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='内容附件表';
CREATE INDEX FK_ATTACHMENT_POST_ID ON CMF_ATTACHMENT (POST_ID);